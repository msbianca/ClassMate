		<?php
			class PastaDao
			{
				...
				public function listarRaiz($idUsuario){
					$query = "SELECT  A.idArquivo, A.idPasta, A.nome AS nomeArquivo, P.nome AS nomePasta, A.descricao AS descArquivo, P. descricao AS descPasta , A.caminhoArquivo FROM tbPasta AS P INNER JOIN tbArquivo AS A ON A.idPasta = P.idPasta INNER JOIN tbParticipacao AS PAR ON PAR.idParticipacao = A.idParticipacao INNER JOIN tbUsuario AS U ON U.idUsuario = PAR.idUsuario WHERE U.idUsuario = $idUsuario AND P.idPasta = 4";
					$stmt = mysql_query($query) or die ($query. mysql_error());
					$result  = array();
					if(mysql_affected_rows() > 0){
					while ($rs = mysql_fetch_array($stmt)){
